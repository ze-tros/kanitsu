...
| 图片索引与缓存 | IndexedDB（目录/图片索引）+ 磁盘缩略图缓存（`userData/thumbcache`）+ 内存 LRU（渲染端/主进程） | 图片规模 <1 万，无需 SQLite |
...
```
原图文件（磁盘）
  → 主进程调度：四级优先级队列（可见 > 当前目录 > 子文件夹 > 全库预热），
    worker 忙时按优先级从高到低取任务
  → worker 线程（4 个，主路径 sharp/libvips）：解码 → 等比缩放 ≤512px → JPEG q80
  → GIF 大图（>256KB）：omggif 抽帧（≤16，均匀采样）→ 逐帧合成（按 disposal 防残影）
    → median-cut 自适应调色板（≤255 色）→ 动画 GIF 缩略图（可动、小体积、无限循环，延迟按采样倍率放大）
  → 缓存：磁盘 `userData/thumbcache`（SHA1 键 = 路径+mtime+size+尺寸+版本；1.5GB 上限按最旧清理）
    + 主进程内存 LRU（512MB）+ 渲染端 LRU（256MB，仅按容量；peek 命中即出图）
```

- 网格可见项最高优先；当前目录全量（P1）、子文件夹（P2）、全库启动预热（P3）逐级低优先级预取（可整体开关）。
- 查看器始终**原始分辨率**（`kanitu-file` 协议流式读取）；切换时先缩略图占位、原图解码完成后换入（无黑屏），并保留已解码相邻原图的 LRU 像素池。
- GIF：≤256KB 原样透传（动画完美、瞬时）；大 GIF 生成可动缩略图；超大条目不入内存缓存、仅落盘。
- 缓存仅按容量 LRU 淘汰；文件被覆盖/移动后键失效自动重建。
...
- 面包屑导航（根到当前目录可点击路径）
- 简体中文界面（UI / 弹窗 / 通知 / 系统对话框 / 错误提示全面中文化）
- 手动固定封面（图片右键“设为封面”，或图包右键“设置封面…”层次化选图：面包屑导航、子文件夹可进入挑选或直接用其封面、当前目录图片网格；`pickCover` 的 `preferredId` 优先，本地记忆）
- 右键菜单（图片：查看/设封面/重命名/复制路径/删除；图包：打开/新建子文件夹/展开收起/重命名/整理/导出/复制路径/删除）
- 图片/图包重命名与删除、新建子文件夹（`entry-ops.ts`，应用内弹框替代原生 confirm/prompt）
- 设置页（通用设置 + 整理规则管理）
- 自定义整理规则（正则匹配去扩展名文件名 + `$1/$2…` 目标模板 + 置信度 + 启停 + 测试预览，localStorage 持久化）
- 深/浅色主题（跟随系统 + 手动切换记忆）
- 侧栏搜索（相册/图片实时过滤）
- 预览模糊 / 隐私打码（右键菜单“设为隐私预览”，逐图绑定：单图或整相册含子文件夹）
- 自建标题栏（最小化/最大化/关闭，移除原生顶栏）
- daisyUI / Tailwind 全面重设计（骨架屏 / 懒加载 / toast / 空态 / 加载动效）
- 整理性能优化（目录/文件名单缓存 + 进度回调）
- 缩略图 worker 管线（4 worker，sharp 主路径 + nativeImage 回退）+ 磁盘持久化缩略图缓存
- 动画 GIF 缩略图（omggif：抽帧/自适应调色板/防残影/循环/采样倍率补偿；小 GIF 原样透传）
- 四级优先级队列 + 全库启动预热/当前目录/子文件夹/封面弹窗预取（可开关）
- 查看器快速切换（缩略图占位 → 原图换入，无黑屏；相邻原图解码像素池 + 鼠标侧键导航）
- 前进/后退导航（浏览器/资源管理器风格，含鼠标侧键）与切换目录滚动位置记忆
- 设置页重构：调试 / 缓存分页（日志等级、预取开关、缓存统计、清除缓存、主进程/渲染端日志）
- 日志体系：主进程 UTF-8 日志文件（分级/时间）+ 渲染端环形缓冲（等级过滤 + 可插拔 sink，移动端可复用）
