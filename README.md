...
Kanitsu是一款跨平台图片管理与查看应用。用户导入文件夹后，应用将图片复制到自己的图包库中，再提供目录浏览、自动整理、封面选择、原图查看和 ZIP 导出。
...
此处的 Portable 指“单 EXE、免安装”，不表示“数据随 EXE 携带”。数据保存在用户首次启动时选定的**数据目录**（图库在其 `albums` 子目录，缩略图缓存放在图库的 `.kanitsu-cache` 子目录，另有 RAW 预览缓存、日志与索引数据），不会随 EXE 移动；系统应用数据目录（`%APPDATA%\Kanitsu`）只保留一个小的 `settings.json` 配置文件。
