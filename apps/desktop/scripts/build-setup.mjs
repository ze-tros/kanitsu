...
// Windows Setup（NSIS 安装包）构建：与 build-portable.mjs 同一条流水线
// （web 构建 -> 主进程编译 -> electron-builder -> 校验 -> 发布目录），
// 区别仅在打包目标：nsis 而非 portable。安装包不解压自身，启动时无需
// 先展开运行时，双击图标到窗口出现的耗时会远小于便携版。
const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const desktopDir = path.resolve(scriptDir, '..');
const repoRoot = path.resolve(desktopDir, '..', '..');
const releaseRoot = path.join(desktopDir, 'release');
const finalOutputDir = path.join(releaseRoot, 'setup');
...
if (process.platform !== 'win32') {
  throw new Error('Windows Setup builds must run on Windows');
}
...
const executableFilename = `Kanitsu-Setup-${desktopPackage.version}-x64.exe`;
...
try {
  ...
  stagingDir = await mkdtemp(path.join(desktopDir, '.setup-staging-'));
  ...
  await run(
    ...
    ],
    {
      ...
      label: `Package Setup x64 (resource editing: ${signAndEditValue})`,
    },
  );
  ...
  publishDir = await mkdtemp(path.join(releaseRoot, '.setup-publish-'));
  await copyFile(
  ...
  await run(
    ...
    [
      ...
      '--write-checksum',
      '--artifact-name', executableFilename,
    ],
    { label: 'Verify Setup artifact and generate SHA-256' },
  );
  ...
  console.log(`\nSetup release ready: ${path.join(finalOutputDir, executableFilename)}`);
  ...
  await removeTemporaryDirectory(stagingDir, 'staging directory');
}
