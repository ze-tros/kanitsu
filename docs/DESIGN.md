...
> 更新于 2026-09-25。本文是当前有效的产品和工程约束，不记录已完成项目的实施过程。
...
整理只作用于应用图库。任何失败都必须可报告，不能静默覆盖已有文件。「移动到…」同样只在图库内进行：图片与目标重名时追加序号，图包整体移动时目标已有同名图包或目标位于其内部则跳过；纯图片移动生成与整理同构的清单，可撤销。
...
浏览（设计演示见 [kanitsu-desktop-redesign-demo.html](kanitsu-desktop-redesign-demo.html)，组件在 `packages/ui/src/desktop/`）：

- 标题栏只放导航（侧栏 / 后退 / 前进）、居中的搜索入口、任务中心与导入；搜索入口打开命令面板（`Ctrl K`），在本机索引里搜图包、图片与命令，图包页内图片只在当前图包（含子目录）里搜，并可一键「在网格中筛选」。主题切换放在设置与命令面板里。
- 侧栏：「全部图包 / 最近导入 / 已固定封面」筛选（与移动端同一语义：已固定 = 设置了固定封面）、带封面缩略的目录树（固定行高分块窗口化）、图库占用与设置入口；可拖宽，窄窗口（< 980px）下为浮层。
- 首页：标题与筛选、继续浏览（本机记录，按图片 id）、图包封面卡片（叠层暗示图包）或列表，排序为最近导入 / 名称 / 图片数 / 占用；列表视图点击表头即按该列排序（图片列表再点同一列切换升降序）。
- 图包页：封面页头（面包屑、智能整理 / 导出 ZIP / 设置封面 / 隐私预览 / 撤销上次操作 / 更多）、横向子图包卡片带（超过 24 个时可展开为虚拟化网格）、吸顶图片工具栏（含子目录、排序、网格 / 按原比例 / 列表、文件名、检查器），吸顶后显示图包名。缩略图尺寸在状态栏滑块或 `Ctrl` + 滚轮调节。
- 选择：勾选框、`Ctrl` / `Shift` 点击、空白处框选、`Ctrl A`；已有选择时单击改为增减选择（与 Android 一致）。批量动作只出现在底部浮动栏（移动到… / 设为封面 / 隐私预览 / 合并为新图包 / 删除；平台实现 `zipSelection` 时另有导出），检查器只展示信息。方向键移动网格焦点，`Enter` 打开，空格勾选，`F2` 重命名，`Del` 删除。
- 检查器（窄于 1180px 时为浮层）按上下文切换：图包 / 图库概览（统计、格式分布、「可按某规则整理」提示）、单图（EXIF 四格摘要 + 全部标签）、多选汇总。
- 导入、整理、导出、移动与批量删除统一进任务中心：进行中可取消（导入 / 整理），整理、移动与合并可撤销最近一次，导入报告与冲突可展开；完成时 Snackbar 提供「撤销 / 打开」。
- 智能整理为两步对话框：选择规则（命中数 + 每组示例映射，默认选命中最多的规则；从检查器提示进入时预选提示的规则）→ 预览分组（可改名，标出并入已有子图包的分组与同名冲突）→ 确认；确认前不移动任何文件。默认只整理直属图片（图包没有直属图片时默认包含子目录），第一步可切换「包含子目录里的图片」。
- 支持把文件夹拖进窗口导入：渲染层只拿到路径，主进程校验它是本地目录、且与数据目录互不包含后，弹原生确认框，用户确认才授权为导入源（与对话框选择同一授权机制）。

查看器：

- 沉浸式：画布铺满窗口，顶部工具栏与底部胶片条在鼠标静止 2.6 秒后淡出；图片信息面板停靠右侧并挤开画布。
- 键盘左右键切图，上下键切同级图包（跳过没有图片的图包），`Home` / `End` 首末张，`0` 适应、`1` 原始大小、`+` / `-` 缩放、`R` 旋转、`I` 信息、`F` 胶片条、`Del` 删除，`Esc` 先收起信息面板再关闭。
- 左右切图与上下切图包在首末循环。支持缩放、平移、旋转（90° / 270° 按对调后的宽高适配）、胶片条（窗口化挂载）、鼠标侧键和相邻原图预取；高宽比超过 2.5 的长图按宽度适配、从顶部开始，滚轮纵向滚动（`Ctrl` + 滚轮仍缩放）；隐私预览的图片同样遮挡，点「本次显示」后本次会话内显示。
- 原图通过受控本地协议加载，不通过 IPC 复制大 Buffer。
- RAW 文件无法被 Chromium 解码：查看器改用主进程完整解码的 JPEG 派生图（`rawcache` 磁盘缓存，同一协议流式服务），派生图最长边 8192、质量 90；工具栏可切换显影 / 直出，完整解码进行中以非阻塞提示展示。
...
- 网格只挂载可视区域及少量 overscan，不能随图片总数线性增长 DOM。桌面端的网格 / 按原比例 / 列表由 `desktop/galleryLayout` 统一计算行几何（网格与列表按算术、按原比例用定长类型数组），框选、方向键与方向预取都基于几何而不读 DOM。
