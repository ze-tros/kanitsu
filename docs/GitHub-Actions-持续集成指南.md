...
| `Windows Portable` | `.github/workflows/windows-portable.yml` | 手动、推送 `v*` tag | `windows-latest` | 打包并校验 Windows x64 Portable | `Kanitsu-Portable-<version>-x64`（EXE + `SHA256SUMS.txt`，保留 14 天）；tag 上由 `release` 作业自动创建并发布 Release |

三个工作流的默认权限是 `contents: read`；只有 `Windows Portable` 的 `release` 作业申请 `contents: write`，用于在 tag 上创建并发布 Release（`build` 作业本身仍是只读）。同一 ref 上有新的运行启动时，旧的未完成运行会被取消（`concurrency.cancel-in-progress`）。
